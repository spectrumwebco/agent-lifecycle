...
func NewDeleteCmd(flags *flags.GlobalFlags) *cobra.Command {
	...
	deleteCmd.Flags().BoolVar(&cmd.Container, "container", true, "If enabled, cleans up the Kled container")
	deleteCmd.Flags().BoolVar(&cmd.Daemon, "daemon", false, "If enabled, cleans up the Kled daemon")
...
func removeContainer(ctx context.Context, workspaceInfo *provider2.AgentWorkspaceInfo, log log.Logger) error {
	log.Debugf("Removing Kled container from server...")
	...
	if workspaceInfo.Workspace.Source.Container != "" {
		log.Infof("Skipping container deletion, since it was not created by Kled")
	} else {
		err = runner.Delete(ctx)
		if err != nil {
			return err
		}
		log.Debugf("Successfully removed Kled container from server")
	}
...
func removeDaemon(workspaceInfo *provider2.AgentWorkspaceInfo, log log.Logger) error {
	...
	log.Debugf("Removing Kled daemon from server...")
	err := agentdaemon.RemoveDaemon()
	if err != nil {
		return errors.Wrap(err, "remove daemon")
	}
	log.Debugf("Successfully removed Kled daemon from server")
