...
import (
	...
	"github.com/spectrumwebco/kled-pro/pkg/platform/form"
	"github.com/spectrumwebco/kled-pro/pkg/platform/project"
...
func (cmd *WorkspaceCmd) Run(ctx context.Context, stdin io.Reader, stdout io.Writer, stderr io.Writer) error {
	...
	if instanceEnv != "" {
		instance := &managementv1.DevPodWorkspaceInstance{} // init pointer - TODO: Update to KledWorkspaceInstance when API is updated
...
func createInstance(ctx context.Context, client client.Client, instance *managementv1.DevPodWorkspaceInstance, log log.Logger) (*managementv1.DevPodWorkspaceInstance, error) {
	...
	updatedInstance, err := managementClient.Loft().ManagementV1().
		...
		Create(ctx, instance, metav1.CreateOptions{}) // TODO: Update to KledWorkspaceInstances when API is updated
	if err != nil {
